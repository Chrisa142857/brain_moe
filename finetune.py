...
from datasets import cv_dataloader_generator
from sklearn.metrics import accuracy_score, precision_recall_fscore_support, roc_curve, auc
from models import Router, BrainMoE, BrainMoEDecoder#brain_net_transformer, neuro_detour, brain_gnn, brain_identity, bolt, graphormer, nagphormer, vanilla_model
...
def main():
    ...
    parser.add_argument('--model', type=str, default = 'brainMoEDecoder')
    parser.add_argument('--max_patience', type=int, default = 50)
    parser.add_argument('--hiddim', type=int, default = 1024)
    parser.add_argument('--lr', type=float, default = 0.0001)
    parser.add_argument('--expert_atlas', nargs='+', default = ['AAL_116','Gordon_333','Shaefer_400'], required=False)
    ...
    experts = MODEL_BANK[args.model](expert_tag=expert_tags, nlayer=args.decoder_layer, head_num=args.decoder_head, hid_dim=args.decoder_hiddim).to(device)
    for experti in range(len(experts.experts)):
        experts.experts[experti].load_state_dict(torch.load(weight_path[experti], map_location='cpu', weights_only=True))
    expert_param = []
    ...
    for foldi in range(_nfold[args.dataname]):
        dataloaders = cv_dataloader_generator(batch_size=args.batch_size, nfold=foldi, dataset=dataset, few_shot=args.few_shot,
                                                                 dname=args.dataname, atlas_name=args.atlas)
        ...
        model = Router(experts=experts.experts, expert_tags=experts.expert_tag, target=args.train_obj, freeze_expert=not args.nofreeze_expert, use_pseudolabel=args.use_pseudolabel, device=device, input_sz=node_sz, nclass=nclass, nlayer=args.nlayer, head_num=args.nhead, hid_dim=hiddim).to(device)
        optimizer = optim.Adam(list(model.parameters())+expert_param, lr=args.lr, weight_decay=args.decay) 
        ...
        for epoch in (pbar := trange(1, args.epochs+1, desc='Epoch')):
            ...
            train(model, experts.experts, device, train_loader, optimizer, args.force_2class)
            print(datetime.now(), 'train done, test start')
            # acc, prec, rec, f1 = eval(model, classifier, device, val_loader)
            scores = eval(model, experts.experts, device, val_loader, args.force_2class)
...
def train(model, experts, device, loader, optimizer, force_2class=False):
    model.train()
    losses = []
    ...
    for step, batch in enumerate(loader):
        ...
        y = model(batch['x'].to(device), experts)
        # if force_2class:
...
def eval(model, experts, device, loader, force_2class=False):
    model.eval()
    # y_true = []
    # y_scores = []
    y_true_dict = {}#{k: [] for k in LOSS_FUNCS}
